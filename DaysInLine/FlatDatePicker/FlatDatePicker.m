...
#define kFlatDatePickerScrollViewDaysWidth SCREEN_WIDTH/3
#define kFlatDatePickerScrollViewMonthWidth SCREEN_WIDTH/3
...
-(id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        // Default fonts.
        self.fontTitle = [UIFont fontWithName:@"HelveticaNeue-Light" size:16.0];
...
-(id)initWithParentView:(UIView*)parentView {
    ...
    _grandParentView = _parentView.superview;
    
    if ([self initWithFrame:CGRectMake(0.0, 0.0, _parentView.frame.size.width, kFlatDatePickerHeight)]) {
        _datePickerMode = FlatDatePickerModeDate;
...
-(void)makeTitle
{
    ...
    NSMutableAttributedString *attributedText = [[NSMutableAttributedString alloc] initWithString:self.labelTitle.text];
    NSRange range = [self.labelTitle.text rangeOfString:@":"];
    ...
    rangeSecondPart.length = self.labelTitle.text.length -range.length - rangeFirstPart.length;
    ...
    [attributedText addAttribute:NSForegroundColorAttributeName value: [UIColor colorWithRed:223/255.0f green:162/255.0f blue:57/255.0f alpha:1.0f] range:rangeFirstPart];
    
    [self.labelTitle setAttributedText:attributedText];
}
...
- (void)buildSelectorLabelsDates {
    ...
    for (NSInteger i = 0; i < _dates.count; i++) {
...
- (void)buildSelectorLabelsHours {
    ...
    for (NSInteger i = 0; i < _hours.count; i++) {
...
- (void)buildSelectorLabelsMinutes {
    ...
    for (NSInteger i = 0; i < _minutes.count; i++) {
...
- (void)buildSelectorLabelsSeconds {
    ...
    for (NSInteger i = 0; i < _seconds.count; i++) {
...
- (void)actionButtonValid {
    ...
    if (_isSelectingEndTime) {
        [self dismiss];
    }else
    {
        [self switchPicker];
        [self dimAllLabelsInArray:_labelsDays];
        [self dimAllLabelsInArray:_labelsMonths];
        [self dimAllLabelsInArray:_labelsYears];
...
-(void)show {
    ...
    if (_parentView != nil) {
        ...
        if (_isInitialized == NO) {
            _parentView.frame = CGRectMake(_parentView.frame.origin.x, _grandParentView.frame.size.height, _parentView.frame.size.width, _parentView.frame.size.height);
            _isInitialized = YES;
        }
        [self dimAllLabelsInArray:_labelsDays];
        [self dimAllLabelsInArray:_labelsMonths];
        ...
        if (self.datePickerMode == FlatDatePickerModeDate ||
            ...
            if ( self.datePickerMode != FlatDatePickerModeMonthAndYear ) {
                NSInteger indexDays = [self getIndexForScrollViewPosition:_scollViewDays];
                [self highlightLabelInArray:_labelsDays atIndex:indexDays];
            }
            
            NSInteger indexMonths = [self getIndexForScrollViewPosition:_scollViewMonths];
            [self highlightLabelInArray:_labelsMonths atIndex:indexMonths];
            
            NSInteger indexYears = [self getIndexForScrollViewPosition:_scollViewYears];
        ...
        if (self.datePickerMode == FlatDatePickerModeTime || self.datePickerMode == FlatDatePickerModeDateAndTime) {
            ...
            NSInteger indexHours = [self getIndexForScrollViewPosition:_scollViewHours];
            [self highlightLabelInArray:_labelsHours atIndex:indexHours];
            
            NSInteger indexMinutes = [self getIndexForScrollViewPosition:_scollViewMinutes];
            [self highlightLabelInArray:_labelsMinutes atIndex:indexMinutes];
            
            NSInteger indexSeconds = [self getIndexForScrollViewPosition:_scollViewSeconds];
        ...
        _parentView.frame = CGRectMake(_parentView.frame.origin.x, _grandParentView.frame.size.height - kFlatDatePickerHeight, _parentView.frame.size.width, _parentView.frame.size.height);
...
-(void)switchPicker {
    ...
    if (_parentView != nil) {
        ...
        [UIView beginAnimations:@"FlatDatePickerSwitch" context:nil];
        [UIView setAnimationDuration:0.3];
        [UIView setAnimationCurve:UIViewAnimationCurveEaseOut];
        [UIView setAnimationDelegate:self];
        [UIView setAnimationDidStopSelector:@selector(animationSwitchDidFinish)];
        
        _parentView.frame = CGRectMake(_grandParentView.frame.origin.x - _parentView.frame.size.width, _grandParentView.frame.size.height - kFlatDatePickerHeight, _parentView.frame.size.width, _parentView.frame.size.height);
        
        [UIView commitAnimations];
    }
    
}
-(void)animationSwitchDidFinish
{
    if (_parentView != nil) {
        _parentView.frame = CGRectMake(_grandParentView.frame.origin.x, _grandParentView.frame.size.height, _parentView.frame.size.width, _parentView.frame.size.height);
        [UIView beginAnimations:@"FlatDatePickerSwitch" context:nil];
        [UIView setAnimationDelegate:self];
        [UIView setAnimationDuration:0.5];
        [UIView setAnimationCurve:UIViewAnimationCurveEaseIn];
        
        _parentView.frame = CGRectMake(_grandParentView.frame.origin.x, _grandParentView.frame.size.height - kFlatDatePickerHeight, _parentView.frame.size.width, _parentView.frame.size.height);
...
- (NSMutableArray*)getHours {
    ...
    for (NSInteger i = 0; i < 24; i++) {
        if (i < 10) {
            [hours addObject:[NSString stringWithFormat:@"0%ld", (long)i]];
        } else {
...
- (NSMutableArray*)getMinutes {
    ...
    for (NSInteger i = 0; i < 60; i++) {
        if (i < 10) {
            [minutes addObject:[NSString stringWithFormat:@"0%ld", (long)i]];
        } else {
...
- (NSMutableArray*)getSeconds {
    ...
    for (NSInteger i = 0; i < 60; i++) {
        if (i < 10) {
            [seconds addObject:[NSString stringWithFormat:@"0%ld", (long)i]];
        } else {
            [seconds addObject:[NSString stringWithFormat:@"%ld", (long)i]];
        }
    }
...
- (void)dimAllLabelsInArray:(NSMutableArray*)labels
{
